...
bool LibQSQL::insertRowIntoTable_noString(const QString& table,const QList<QString>& columns,const QList<QString>& items )
{
...
bool LibQSQL::insertRowIntoTable_allString(const QString& table,
                                           const QList<QString>& columns,const QList<QString>& items )
{
    QString queryString=QString("INSERT INTO %1(").arg(table);
    for(auto& i:columns)
    {
...
void L_library::LibQSQL::printLastError() const
{
    if(this->query!=nullptr)
    {
        qDebug()<<"last database error:"<<db->lastError();
