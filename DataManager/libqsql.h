...
public:
   ...
   bool empty() const {if(db!=nullptr)return db->tables().size()==0;return true;};
   ...
   bool insertRowIntoTable_allString(const QString& table,const QList<QString>& columns,const QList<QString>& items );
   bool insertRowIntoTable_noString(const QString& table,const QList<QString>& columns,const QList<QString>& items );
